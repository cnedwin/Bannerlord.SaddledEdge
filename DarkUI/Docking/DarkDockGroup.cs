...
namespace DarkUI.Docking
{
    ...
    public class DarkDockGroup : Panel
    {
        ...
        private void BuildTabs()
        {
            ...
            var closeEnabled = this.DockPanel.CloseButtonEnabled;

            var closeButtonSize = closeEnabled ? DockIcons.close.Width : 0;

            // Calculate areas of all tabs
            var totalSize = 0;

            var orderedContent = _contents.OrderBy(c => c.Order).ToArray();
            ...
            if (DockArea == DarkDockArea.Document)
            {
                ...
                {
                    var tab = _tabs[content];
                    var closeRect = !closeEnabled ? Rectangle.Empty :
                            new Rectangle(tab.ClientRectangle.Right - 7 - closeButtonSize - 1,
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            foreach (var tab in _tabs.Values)
            {
                ...
                if (tab.CloseButtonRectangle.Width > 0)
                {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            foreach (var tab in _tabs.Values)
            {
                ...
                if (rect.Contains(e.Location))
                {
                    ...
                    {
                        if (this.DockPanel.CloseButtonEnabled)
                            tab.DockContent.Close();
                        return;
                    }

                    var closeRect = RectangleToTabArea(tab.CloseButtonRectangle);
                    if (this.DockPanel.CloseButtonEnabled && closeRect.Contains(e.Location))
                    {
