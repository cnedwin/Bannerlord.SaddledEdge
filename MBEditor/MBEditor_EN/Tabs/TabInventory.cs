        ...
        private void InitializeList()
        {
            ...
            lstInvEquipment.AllColumns.Add(new OLVColumn
            {
                ...
                AspectPutter = (item, value) =>
                {
                    ...
                    this.Player.PartyBelongedTo.ItemRoster.AddToCounts(this.Player.PartyBelongedTo.ItemRoster.GetItemAtIndex((int)item), diff);
                },
            });
            lstInvEquipment.AllColumns.Add(new OLVColumn
            {
        ...
        private void lstInvEquipment_CellEditStarting(object sender, CellEditEventArgs e)
        {
            if (e.Column.Text == "Modifier")
            {
                ...
                cb.SelectedIndexChanged += (cb_sender, cbe) =>
                {
                    ...
                    var lastSelElem2 = this.Player.PartyBelongedTo.ItemRoster.GetItemAtIndex(lastSelIdx);
                    //这里有问题，还没修好
                    this.Player.PartyBelongedTo.ItemRoster.AddToCounts(lastSelElem2, 1);
                };
                e.Control = cb;
            }
        ...
        private void AddCurrentSelect()
        {
            if (this.numInvItemCount.Value != decimal.Zero)
            {
                foreach(ItemObject item in lstAllItems.SelectedObjects)
                {
                    ...
                    {
                        this.Player.PartyBelongedTo.ItemRoster.AddToCounts(rosterItem.EquipmentElement, number);
        ...
        private void btnEnsureFood_Click(object sender, EventArgs e)
        {
            ...
            foreach (var food in foodlist)
            {
                ...
                while (curidx >= 0)
                {
                    ...
                    var itemelem2 = this.Player.PartyBelongedTo.ItemRoster.GetItemAtIndex(curidx);
                    ...
                    else
                    {
                        if (itemelem.Amount < minCount)
                            this.Player.PartyBelongedTo.ItemRoster.AddToCounts(itemelem2, Math.Max(itemelem.Amount, minCount));
                    }
