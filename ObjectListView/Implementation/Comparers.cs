    ...
    public class ModelObjectComparer : IComparer, IComparer<object>
    {
        ...
        public static int CompareValues(object x, object y)
        {
            ...
            try
            {
                ...
                if (tx == ty)
                {
                    if (tx == typeof(string)) // most common pattern
                        return CompareStrings((string)x,(string)y);
                    if (tx == typeof(float)) // common pattern
                ...
                if (x is IComparable && y is string)
                {
                    ...
                    if (ox != null)
                        return (ox as IComparable).CompareTo(y);
                }
                else if (y is IComparable && x is string)
                {
                    object oy = Convert.ChangeType(x, tx);
                    if (oy != null)
                    {
                        if (x is IComparable)
                            return (oy as IComparable).CompareTo(x);
                        else
                            return -((oy as IComparable).CompareTo(oy));
                    }
                }
            ...
            try { return CompareStrings(x?.ToString(), y?.ToString()); }
            catch { return -1; }
        }
        public static bool IsNumeric(object t)
        {
        ...
        public static int CompareValues(object x, object y, object tolerance)
        {
            if (tolerance == null) return CompareValues(x, y);
